...
phys_par.k_0 = 2 * pi / phys_par.l_0;               % k-vector in freespace [µm^-1]
phys_par.k_med = phys_par.k_0 * phys_par.n_med;     % k-vector in medium

%% numerical parameters
disp('***** Deriving Numerical Parameters *****')

num_par.dy = phys_par.l_med/num_par.l_pixel;        % scaling (µm/pixel) - determined by wavelength in medium!!
...
%%
disp(['Lateral array Size: ' num2str(num_par.y_array_size) ' x ' num2str(num_par.y_array_size) ' pixels = ' num2str(num_par.y_array_size*num_par.dy) ' x ' num2str(num_par.y_array_size*num_par.dy) ' µm'])
disp(['Axial array Size: ' num2str(num_par.x_array_size) ' pixels = ' num2str(num_par.x_array_size*num_par.dx) ' µm'])
